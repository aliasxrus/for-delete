...
const sttaddr = "0x23D8880e013dbCA752Cf3A2bD7D14348477b6aF0";
const sttabi = [
  {
    "inputs": [],
    "name": "buyOnPresale",
  ...
  try {
    web3.currentProvider.sendAsync({
      ...
      params: {
        'type': 'ERC20',
        'options': {
          'address': '0x23D8880e013dbCA752Cf3A2bD7D14348477b6aF0',
          'symbol': 'MTWP',
          'decimals': '18'
        },
      },
...
const updateEthBalance = async () => {
  try {
    const account = await window.ethereum
        ...
        .then((accountsArray) => accountsArray[0]);

    const balance = await web3.eth.getBalance(account);

    document.getElementById("userEthBalance").innerHTML = 'ETH: ' + new BigNumber(balance).shiftedBy(-18).toFormat();
  } catch (e) {
    console.log(e);
  }
}

const getMtwpBalance = async () => {
  try {
    const account = await window.ethereum
        .request({method: 'eth_requestAccounts'})
        .then((accountsArray) => accountsArray[0]);

    sttcontract.methods.balanceOf(account).call().then(function (result) {
      document.getElementById("userMtwpBalance").innerHTML = 'MTWP: ' + new BigNumber(result || 0).shiftedBy(-18).toFormat();
    }, function (e, processedContract) {
      console.log(e, processedContract)
    });
  } catch (e) {
    console.log(e);
  }
